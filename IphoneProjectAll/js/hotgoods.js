...
$(".list li").eq(0).on("click",function(){
    ...
    $("#search-kuan").on("keyup",function(event){
		...
		window.onload=function () {
			...
			if(cart_id!==null){
				window.shop.api.fetchGoodsListByCatId(cart_id,page,20,function (json) {
					console.log(json);
					if (json.code===1) {
						var oDivssss=document.createElement('div');
						oDivssss.style.width='100'+'%';
						oDivssss.style.height='50'+'px';
						oDivssss.style.textAlign='center';
						oDivssss.style.fontSize='30'+'px';
						oDivssss.innerText=json.message;
						document.body.appendChild(oDivssss)
					}
					var data=json.data;
					for(var i=0;i<data.length;i++){
						var obj=data[i];
